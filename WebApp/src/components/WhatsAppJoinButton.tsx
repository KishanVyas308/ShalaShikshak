...
import type { WhatsAppLink } from '../services/whatsappAPI';

// Global state for WhatsApp link
...
const WhatsAppJoinButton: React.FC<WhatsAppJoinButtonProps> = ({ 
  className = '', 
  variant = 'floating',
  size = 'md' 
}) => {
    ...
    return (
      ...
      >
        <button
          ...
          className="group relative w-14 h-14 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-full shadow-lg hover:shadow-xl cursor-pointer transition-all duration-300 transform hover:scale-110 active:scale-95"
          title={activeLink.description || `Join ${activeLink.name}`}
        >
          {/* Pulse animation */}
